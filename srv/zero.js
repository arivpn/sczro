...
var btngrass = {
contentText: `${anu}`,
footerText: 'Created By Arie Tube',
buttons: punten,
headerType: 1
}
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `Jangan Lupa Subscribe Arie Tube`)
fs.unlinkSync(ran)
})
break
case 'stickerwm':
case 'swm':
if (args.length == 0) return reply(`Teks Nya Mana ?\nContoh: ${prefix + command} Arie | Gantenk`)
...
case 'youtube':
teks =
`Nih Youtube Owner Ku Jangan Lupa Di Subscribe Ya https://youtube.com/ArieTube`
zero.sendMessage(from, teks, text, {quoted: ftrol})
break
case 'instagram':
teks =
`Nih Instagram Owner Ku Jangan Lupa Di Follow Ya https://instagram.com/Arie_Tube`
zero.sendMessage(from, teks, text, {quoted: ftrol})
break
case 'tiktok':
teks =
...
await zero.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: ftrol, caption: 'Nih Jangan Lupa Subscribe Arie Tube'})
break
case 'pinterest':
case 'pin':
if (!q) return reply("")
pint = await pinterest (q)
zero.sendMessage(from, {url: `${pint[Math.floor(Math.random() * pint.length)]}`}, image, {quoted: ftrol, thumbnail: Buffer.alloc(0), caption: "Nih Jangan Lupa Subscribe Arie Tube"})
break
case 'herolist':
...
case 'horrorblood':
case 'thunder':
if (args.length == 0) return reply(`Teksnya Mana ?\nContoh : ${prefix + command} Arie Tube`)
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey1}&text=${ini_txt}`).then((gambar) => {
zero.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `DONE NIH JANGAN LUPA SUBSCRIBE ARIE Tube`, quoted : ftrol})
})
break
case 'pornhub':
case 'glitch':
...
zeroyt7.sendMessage(from, gambar, image, { thumbnail: Buffer.alloc(0), caption: `DONE NIH JANGAN LUPA SUBSCRIBE Arie Tube`, quoted : ftrol })
})
break
case 'imagelist':
case 'listimage':
...
buttonsMessage = {footerText:'©Created By Arie Tube', imageMessage: imageMsg,
contentText:`DONE NIH JANGAN LUPA SUBSCRIBE Arie Tube`,buttons,headerType:4}
...
imageMsg = (await zero.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
buttonsMessage = {footerText:'©Created By Arie Tube', imageMessage: imageMsg,
contentText:`DONE NIH JANGAN LUPA SUBSCRINE Arie Tube`,buttons,headerType:4}
prep = await zero.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
zero.relayWAMessage(prep)
break
case 'bj':
...
e = String(e)
if (!e.includes("this.isArie") && !e.includes("jid")) {
